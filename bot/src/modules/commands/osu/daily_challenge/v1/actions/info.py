    ...
    for _ in range(MAX_ATTEMPTS):
        try:
            text = (
                ...
                "Через 24 часа после последнего запроса челленджа Tier <b>сбрасывается</b> до 1\n\n"        
                "Челлендж можно пропустить, но на это потратятся очки\n\n"
                "Если после получения челленджа пройдет 24 часа, он просто исчезнет, очки не потеряются\n\n"
            )
