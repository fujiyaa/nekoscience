...
async def  start_score_compare(update, context, user_request=True):
    if user_request: await log_all_update(update)
    asyncio.create_task(score_compare(update, context, user_request))

async def score_compare(update: Update, context: ContextTypes.DEFAULT_TYPE, user_request=True):    
    try:
