...
def get_context_keyboard(
    ...
    if map_id1:
        keyboard.append([
            InlineKeyboardButton(
                ...
                callback_data=f"assets_ctx:map:{map_id1}:{origin_user_id}"
            )
        ])
    ...
    if map_id2:
        keyboard.append([
            InlineKeyboardButton(
                ...
                callback_data=f"assets_ctx:map:{map_id2}:{origin_user_id}"
            )
        ])
    ...
    keyboard.append([
        InlineKeyboardButton(
            "✖️ Отмена",
            callback_data=f"assets_ctx:cancel:0:{origin_user_id}"
        )
    ])
