...
from .send_assets import send_assets
...
async def start_beatmap_assets(update, context):
    await log_all_update(update)
    asyncio.create_task(beatmap_assets(update, context))
async def beatmap_assets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        bg_file_path = os.path.join(base_path, bg_path)

        bot_msg = await send_assets(
            update, 
            context, 
            bg_file_path
        )
