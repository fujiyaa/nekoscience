...
async def get_context_keyboard(
    message_context, 
    ...
    if map_id1:
        keyboard.append([
            InlineKeyboardButton(
                ...
                callback_data=f"sicxt:map:{map_id1}:{origin_user_id}:{origin_msg_id}"
            )
        ])
    ...
    if map_id2:
        keyboard.append([
            InlineKeyboardButton(
                ...
                callback_data=f"sicxt:map:{map_id2}:{origin_user_id}:{origin_msg_id}"
            )
        ])
    ...
    for title, ids, diffs in map_titles:
        keyboard.append([
            InlineKeyboardButton(
                title or "Mapset",
                callback_data=f"sicxt:ignore:0:{origin_user_id}:0"
            )
        ])
        ...
        for map_id, diff in zip(ids, diffs):
            row.append(
                InlineKeyboardButton(
                    diff,
                    callback_data=f"sicxt:map:{map_id}:{origin_user_id}:{origin_msg_id}"
                )
            )
            if len(row) == 3:
                keyboard.append(row)
    ...
    keyboard.append([
        InlineKeyboardButton(
            "✖️ Отмена",
            callback_data=f"sicxt:cancel:0:{origin_user_id}:{origin_msg_id}"
        )
    ])
...
#             InlineKeyboardButton(
#                 map_title1 or f"map {map_id1}",
#                 callback_data=f"sicxt:map:{map_id1}:{origin_user_id}"
#             )
#         ])
...
#             InlineKeyboardButton(
#                 map_title2 or f"map {map_id2}",
#                 callback_data=f"sicxt:map:{map_id2}:{origin_user_id}"
#             )
#         ])
...
#         InlineKeyboardButton(
#             "✖️ Отмена",
#             callback_data=f"sicxt:cancel:0:{origin_user_id}"
#         )
#     ])
