...
if not DEV:
    ...
    OSU_CLIENT_ID = os.getenv("DEV_OSU_CLIENT_ID_CHAT_AUTH")
    OSU_CLIENT_SECRET = os.getenv("DEV_OSU_CLIENT_SECRET_CHAT_AUTH")
    OSU_REDIRECT_URI = os.getenv("DEV_OSU_REDIRECT_URI_CHAT_AUTH")

file_id = "file_chat_verified" 
...
@router.get("/chat/oauth/callback")
async def oauth_callback(request: Request, code: str = None):
    ...
    return templates.TemplateResponse("chat_auth_code.html", {
        "request": request,
        "username": username,
        "code": new_code
    })